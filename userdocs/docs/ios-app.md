...
- **Adherence** -- once a protocol has a run and at least one closed day, the protocol detail screen shows an adherence summary ("83.3% adherence · 20 done · 2 skipped · 2 missed"). Skipped doses are excluded from the adherence percentage -- a deliberate skip is not treated as a failure. Before a run's first closed day, this reads "No closed days yet." If every closed day so far was skipped, there's nothing to compute a percentage over even though days have closed -- this reads "No doses counted yet · N skipped" instead. A paused run still shows its adherence and dose log (frozen as of when it was paused); it's only excluded from the dashboard's missed-doses count while paused.
