...
async fn test_list_sleep_records_with_date_filter() {
    ...
    // Two records: one 10 days ago (inside the filter window) and one today (outside).
    let today = chrono::Utc::now().date_naive();
    let ten_days_ago = today - chrono::Duration::days(10);

    for (date, source) in [(ten_days_ago.to_string(), "manual"), (today.to_string(), "garmin")] {
        let body = json!({
            "date": date,
...
async fn test_cross_user_isolation() {
    ...
    // User B tries to GET the record — must be 404, not 200.
    let get_resp = app
        .app
        .clone()
        .oneshot(common::auth_request(
    ...
    // User B tries to DELETE the record — must NOT return 204 (success with data).
    let delete_resp = app
        .app
        .clone()
        .oneshot(common::auth_request(
        ...
        .unwrap();
    assert!(
        delete_resp.status() == 404 || delete_resp.status() == 204,
        "expected 404 or 204, got {}",
        delete_resp.status()
    );

    // Confirm the record still exists for user A (delete by B must have been a no-op).
