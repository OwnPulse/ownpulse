...
/// PATCH /admin/users/:id/status -- enable or disable a user (admin only, can't disable self).
...
/// Generate a cryptographically random 16-character base62 invite code.
...
) -> Result<(axum::http::StatusCode, Json<InviteResponse>), ApiError> {
...
mod tests {
    ...
    #[test]
    fn invite_code_is_16_chars_base62() {
        let code = generate_invite_code();
