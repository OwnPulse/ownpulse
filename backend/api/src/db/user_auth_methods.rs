...
/// Find a user by their provider + provider_subject inside a transaction.
///
/// Used during account linking to check whether a Google/Apple subject is
/// already linked to another user within the same transaction boundary.
pub async fn find_by_provider_subject_tx(
