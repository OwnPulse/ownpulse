...
use crate::models::protocol::{
    CreateProtocol, LogDoseRequest, ProtocolDoseRow, ProtocolLineResponse, ProtocolLineRow,
    ProtocolListItem, ProtocolResponse, ProtocolRow, SkipDoseRequest, TodaysDoseItem,
    UpdateProtocol,
};
    ...
    let protocol = sqlx::query_as::<_, ProtocolRow>(
        "INSERT INTO protocols (user_id, name, description, start_date, duration_days)
         ...
         RETURNING id, user_id, name, description, start_date, duration_days,
                   status, share_token, share_expires_at, created_at",
    )
    .bind(user_id)
    .bind(&req.name)
