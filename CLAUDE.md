...
| `genetic_records` | structured | SNP variants from 23andMe/AncestryDNA/VCF uploads |
| `snp_annotations` | structured | Reference SNP annotations (ClinVar, PharmGKB) for interpretation |
...
- **Genetic interpretation must cite sources.** Every SNP annotation must reference its source database (ClinVar, PharmGKB, SNPedia) and evidence level. Never present raw associations as medical advice — always include disclaimers and confidence levels.
